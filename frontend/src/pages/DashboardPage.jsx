            ...
            {statCards.map((card) => (
              <div key={card.key} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{loading ? '—' : stats[card.key]}</p>
                </div>
              </div>
            ))}
          </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {recentUsers.map((u) => (
                      <tr key={u._id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${u.isActive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
