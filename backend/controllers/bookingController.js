...
const getBookingById = async (req, res) => {
  try {
    ...
    const schedules = await PaymentSchedule.find({ booking: booking._id }).sort({ dueDate: 1 })
    const payments = await Payment.find({ booking: booking._id }).sort({ date: -1 })
...
const createBooking = async (req, res) => {
  try {
    ...
    const booking = new Booking({ ...req.body, bookedBy: req.user._id, createdBy: req.user._id })
    await booking.save()
